...
app.post('/session/init', async () => {
    const id = randomUUID();
    sessions.set(id, { id, balance: 100 });
    return { ok: true, sessionId: id, balance: 100 };
});
app.post('/spin', async (req, reply) => {
    const body = req.body;
    const session = sessions.get(body.sessionId);
    if (!session)
        return reply.code(404).send({ ok: false, error: 'Session not found' });
    if (body.bet <= 0 || body.bet > 50 || session.balance < body.bet) {
        return reply.code(400).send({ ok: false, error: 'Invalid bet or insufficient balance' });
    }
    const stops = [0, 1, 2, 3, 4].map((i) => Math.floor(Math.random() * strips[i].length));
